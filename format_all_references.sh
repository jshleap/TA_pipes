...
format_it(){
  pyndada=/home/jshleap/my_gits/pyndada
  prefix=${1%%.withoutliers}
  krak=$(echo $prefix| sed 's/\./_/')
  PROTAX=/home/jshleap/Programs/protaxA/scripts
  priors=10,5,1
  # fix most names
  python3 ${pyndada}/fix_taxonomy.py ${1} > tmp && mv tmp ${1}
  # format kraken
  if ls -U ${krak}/*.k2d 1> /dev/null 2>&1
  then
  ...
  clean
  if ls -U ${krak}/*_protax_intermediateFiles.tar.bz2 1> /dev/null 2>&1
  then
...
export PYTHONPATH=$PYTHONPATH:~/my_gits/A2G:~/my_gits/justblast
export -f format_it
# align them all
for mock in FSIS SIS MIS PSS
do
  if [[ ! -f ${mock}_realized.reference_aligned.withoutliers ]]
    then
